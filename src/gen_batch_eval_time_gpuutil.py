...
def get_gpu_metrics(gpu_index):
    ...
    metrics = [int(metrics[0])] + [int(metric.strip('%')) for metric in metrics[1:3]] + [int(metrics[3].strip(' MiB'))]
...
def monitor_gpu(gpu_index=0):
    with open(gpu_util_csv, 'w', newline='') as csvfile:
        ...
        csv_writer.writerow(['timestamp', 'temperature.gpu', 'utilization.gpu [%]', 'utilization.memory [%]', 'memory.used [MiB]'])
...
if __name__ == "__main__":
    ...
    gpu_util_csv = os.path.join(args.output_dir, f'nvidia_smi_gpuutil.csv')
    time_stat_csv = os.path.join(args.output_dir, f'latency_throughput_gpuutil.csv')
    gen_log_path = os.path.join(args.output_dir, f'gen_output_gpuutil.txt')
