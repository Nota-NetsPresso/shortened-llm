...
COMPRESSED_LLMPRUNER_ID="./src/baselines/llm-pruner/llama-1-5.4b_lora_merged/pytorch_model.bin"
    ...
    with gr.Blocks(theme="nota-ai/theme") as demo:
        ...
        with gr.Row():
            ...
            with gr.Accordion("Advanced Settings", open=True):
                with gr.Row():
                    max_new_tokens = gr.Slider(32, 256, label="Output Tokens", value=256, step=32)
                    ...
                    temperature = gr.Slider(0.0, 1.0, label="Temperature", value=1.0, step=0.05)
        ...
        with gr.Row():
            ...
            with gr.Column(variant="panel", scale=32):
                ...
                with gr.Column():
                    gen_orig_button = gr.Button(value="Generate with Original Model", variant="primary")
                    orig_model_test_time = gr.Textbox(value="", label="Test Time; Throughput (Batch Size)")
            ...
            with gr.Column(variant="panel", scale=32):
                ...
                with gr.Column():
                    ...
                    llmprn_test_time = gr.Textbox(value="", label="Test Time; Throughput (Batch Size)")
                    with gr.Accordion("Output Text", open=True):
            ...
            with gr.Column(variant="panel", scale=32):
                ...
                with gr.Column():
                    ...
                    ours_test_time = gr.Textbox(value="", label="Test Time; Throughput (Batch Size)")
                    with gr.Accordion("Output Text", open=True):
        ...
        orig_model_outputs = [orig_model_output, orig_model_test_time, orig_model_output_kr]
        gen_orig_button.click(servicer.infer_orig_model, inputs=inputs, outputs=orig_model_outputs)

        llmprn_outputs = [llmprn_output, llmprn_test_time, llmprn_output_kr]
        gen_llmprn_button.click(servicer.infer_llmpruner, inputs=inputs, outputs=llmprn_outputs)
