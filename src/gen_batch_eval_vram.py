...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default="results_efficiency/llama-1-7b/batch_gen_out128_bs4")        
    ...
    inputs = tokenizer([args.input_prompt]*args.batch_size,
                       return_tensors="pt", padding=True)["input_ids"].to(args.device)
    input_len=inputs[0].size(0)

    # Set log path
    os.makedirs(args.output_dir, exist_ok=True)
    mem_stat_csv = os.path.join(args.output_dir, f'memory_tracemalloc.csv')
    time_stat_csv = os.path.join(args.output_dir, f'latency_throughput_tracemalloc.csv')
    gen_log_path = os.path.join(args.output_dir, f'gen_output_tracemalloc.txt')
            ...
            if 'cuda' in args.device:
                ...
                with torch.no_grad():
                    generation_output = model.generate(
                        ...
                        min_length=(input_len+args.max_seq_len), # forced output length (to avoid <EOS> sampling)
        ...
        if i < args.num_warmup_runs: # no record for warmup runs
            ...
            for b_idx, s in enumerate(output):
                ...
                with open(gen_log_path, 'a', encoding='utf8') as f:
                    f.write(f"=== output {i} b{b_idx} | leng gen {tmp_size} + input {input_len}\n")
                    f.write(f"{s}\n")
                    print(f"=== output {i} b{b_idx} | leng gen {tmp_size} + input {input_len}\n")
                    print(f"{s}\n")
    ...
    with open(time_stat_csv, 'w') as logfile:
        ...
        logwriter.writerow(['time_mean(sec)','time_std','th_mean(tokens/s)','th_std','mem','out_len','in_len','nbatches','batchsz','nparam'])
        logwriter.writerow([time_mean,time_std,throughput_mean,throughput_std,mem,args.max_seq_len,input_len,nbatches,args.batch_size,nparams])
        logwriter.writerow(time_list)

    with open(mem_stat_csv, 'w') as logfile:
        logwriter = csv.writer(logfile, delimiter=',')
        logwriter.writerow(['mem_gpu_mean','mem_gpu_std','mem_cpu_mean','mem_cpu_std','out_len','in_len','nbatches','batchsz','nparam'])
        logwriter.writerow([mem_gpu_mean,mem_gpu_std,mem_cpu_mean,mem_cpu_std,args.max_seq_len,input_len,nbatches,args.batch_size,nparams])
