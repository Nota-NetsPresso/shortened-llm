...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default="output_block_sensitivity/llama-1-7b/magnitude_n10")
    ...
    result_csv_weight = os.path.join(args.output_dir, f"weight_score.csv")  
    result_csv_block = os.path.join(args.output_dir, f"block_score_all.csv")  
    result_csv_block_detail = os.path.join(args.output_dir, f"block_score_detail.csv")  
    result_csv_block_sort = os.path.join(args.output_dir, f"block_score_sorted.csv")  
            ...
            for k, param in model.named_parameters():
                if param.requires_grad and 'weight' in k and 'embed_tokens' not in k:                    
                    ...
                    if 'proj' in k or 'lm_head' in k: # output_dim x input_dim
                        weight_imp=param.data.clone().float().abs().pow(norm_power).sum(1) # [output_dim]
