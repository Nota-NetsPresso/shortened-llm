...
from utils import set_seed, get_model, get_block_pruned_network, count_params
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default="output_block_sensitivity/llama-1-7b/ppl_n10")
    ...
    unsorted_output_path = os.path.join(args.output_dir, f'all_ppl_unsorted.csv')    
    sorted_output_path = os.path.join(args.output_dir, f'all_ppl_sorted.csv')    
    block_order_path = os.path.join(args.output_dir, f'block_order.csv') 
    ...
    if not os.path.exists(block_order_path):    
        ...
        example_prompts = get_examples('bookcorpus', tokenizer, 
                                    args.num_calib_data, seq_len = args.max_seq_len).to(args.device)
        for block_idx in range(model_orig.config.__getattribute__("num_hidden_layers")):           
            csv_log_path = os.path.join(args.output_dir, f'ppl_block{block_idx}_removed.csv')
            ...
            with open(csv_log_path, 'w') as logfile:
                ...
                logwriter.writerow(['removed_block', 'ppl_bookcorpus', 'num_calib_data', 'params'])
                logwriter.writerow([block_idx, ppl, args.num_calib_data, count_params(model)])
        ...
        block_order = []  
        with open(sorted_output_path, 'w') as logfile, open(block_order_path,'w') as logfile_order:
            logwriter = csv.writer(logfile, delimiter=',')
            logwriter.writerow(['removed_block', 'ppl_bookcorpus', 'num_calib_data', 'params'])
